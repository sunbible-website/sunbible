...
def index():
    ...
    return render_template('index.html', title='SUN Translation Resources', navindex=True, menu_data=menu_data)
...
def where():
    with open('./json/gl_active_list.json') as file:
        menu_data = json.load(file)
    session.pop('name', None)
    return render_template('where-org.html', title='projects', menu_data=menu_data)
...
def terms():
    with open('./json/gl_active_list.json') as file:
        menu_data = json.load(file)
    session.pop('name', None)
    return render_template('terms-conditions.html', title='Terms and Conditions', menu_data=menu_data)
...
def privacy():
    with open('./json/gl_active_list.json') as file:
        menu_data = json.load(file)
    session.pop('name', None)
    return render_template('privacy-policy.html', title='Privacy Policy', menu_data=menu_data)


# This is the "status.html" route.
@app.route('/status')
def status():
    with open('./json/gl_active_list.json') as menufile:
        menu_data = json.load(menufile)
    with open('./json/lang_status.json','r') as file:
        json_data = file.read()
    html_table = json_to_html_table(json_data)
    session.pop('name', None)
    return render_template('status.html', title='Translation Status',html_table=html_table, menu_data=menu_data)
...
def partners():
    ...
    html_table = json_to_html_table(json_data)
    session.pop('name', None)
    return render_template('partners.html', title='partners',html_table=html_table, menu_data=menu_data)
...
def resource_page(code):
    ...
    session.pop('name', None)
    return render_template('resource.html', title='Resource Files', html_content=html_content,code=code, menu_data=menu_data)
