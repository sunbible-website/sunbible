...
def index():
    ...
    return render_template('index.html', title='SUN Translation Resources', navindex=True, json_data = menu_data )
...
def where():
    #with open('./json/gl_active_list.json') as file:
      #menu_data = json.load(file)
    session.pop('name', None)
    return render_template('where-org.html', title='projects', json_data = menu_data )
...
def terms():
    #with open('./json/gl_active_list.json') as file:
      #menu_data = json.load(file)
    session.pop('name', None)
    return render_template('terms-conditions.html', title='Terms and Conditions', json_data = menu_data )
...
def privacy():
    #with open('./json/gl_active_list.json') as file:
      #menu_data = json.load(file)
    session.pop('name', None)
    return render_template('privacy-policy.html', title='Privacy Policy', json_data = menu_data )
...
def status():
    ...
    session.pop('name', None)
    return render_template('status.html', title='Translation Status',html_table=html_table, json_data = menu_data )
...
def partners():
    ...
    html_table = json_to_html_table(table_data)
    session.pop('name', None)
    return render_template('partners.html', title='partners',html_table=html_table, json_data = menu_data )
...
def resource_page(code):
    ...
    session.pop('name', None)
    return render_template('resource.html', title='Resource Files', navindex=True, html_content=html_content,code=code, json_data = menu_data )
